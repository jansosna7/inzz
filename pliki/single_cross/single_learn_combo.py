...
def eval_genome(genome, config):
    ...
    if(fitness_3 < -147):
            return fitness_3 - 1000
    fitness_4 = repeat_run_sim(genome, config, "_5")
    if(fitness_3 < -150):
        return fitness_4 - 500
    fitness_5 = repeat_run_sim(genome, config, "_4")
    return fitness_5

def learn(prefix, data_value, stepThreshold):
    random.seed(52 + prefix)
    # NEAT configuration
...
def repeat_learn():
    for data_value in ["VehicleNumberAndHaltingNumber"]:
        for stepThreshold in [3000]:
            for prefix in [211,212,213,214]:
