...
def learn(prefix, data_value, learningStepCount):
    ...
    try:
        ...
        pop.add_reporter(neat.Checkpointer(generation_interval=60, time_interval_seconds=1800,
                         filename_prefix=os.path.join("results", "neat-checkpoint"+PDL)+"-"))
    ...
    for data_value in ["VehicleNumber", "HaltingNumber","VehicleNumberAndHaltingNumber","HaltingNumberAndMeanSpeed","All"]:
        for learningStepCount in [1,3]:
            for prefix in [11,12,13]:
                learn(prefix, data_value, learningStepCount)
