...
def test():
    for data_value,stepThreshold,PDSG  in [["VehicleNumberAndHaltingNumber", 4000 ,"14_data_VehicleNumberAndHaltingNumber_stepThreshold_4000_gen_80"],
                                           ["All", 3000, "14_data_All_stepThreshold_3000_gen_80"]]:
        ...
        config.testStepCount = 1
        config.id = PDSG

        try:
            filename = os.path.join("results_h3","winner_"+PDSG)
            with open(filename, 'rb') as f:
                winner = pickle.load(f)
                for num in ["_1","_2","_3","_4","_5"]:
                    result = run_sim(winner, config, num)
                    with open(os.path.join("emissions_test","results.txt"), 'a') as file:
...
if __name__ == "__main__":
    ...
    os.makedirs("testing_h3", exist_ok=True)
