...
def run_sim(genome, config, num):
    ...
    learningStepCount = config.learningStepCount
...
def learn(prefix, data_value, learningStepCount, epsilon, pop_size):
    random.seed(42)
    # NEAT configuration
    ...
    #file name prefix
    PDLE = str(prefix) + "_pop_" + str(pop_size) + "_data_" + data_value + "_step_" + str(learningStepCount) + "_eps_"+ str(epsilon)
    ...
    try:
        ...
        pop.add_reporter(neat.Checkpointer(generation_interval=60, time_interval_seconds=1800,
                         filename_prefix=os.path.join("results", "neat-checkpoint"+PDLE)+"-"))
        ...
        for generations in [5,15,20]:
            gen += generations
            winner = pop.run(pe.evaluate, generations)
            PDLEG = PDLE + "_gen_" + str(gen)

            # save some data
            with open(os.path.join("results", "winner_"+PDLEG), 'wb') as f:
                pickle.dump(winner, f)

            with open(os.path.join("results", "stats_"+PDLEG), 'wb') as f:
                pickle.dump(stats, f)

            visualize.plot_stats(stats, ylog=False, view=False, filename=os.path.join(
                "results", "fitness_"+PDLEG+".svg"))
            visualize.plot_species(stats, view=False, filename=os.path.join(
                "results", "speciation_"+PDLEG+".svg"))
            visualize.draw_net(config, winner, view=False, filename=os.path.join(
                "results", "drawing_"+PDLEG))
...
def repeat_learn():
    #for data_value in ["VehicleNumber", "HaltingNumber", "MeanSpeed","VehicleNumberAndHaltingNumber","HaltingNumberAndMeanSpeed","MeanSpeedAndVehicleNumber","All"]:
    for data_value in ["VehicleNumberAndHaltingNumber","HaltingNumberAndMeanSpeed","MeanSpeedAndVehicleNumber","All"]:
        for epsilon in [0.1, 0.01]:
            for learningStepCount in range(1, 4):
                for pop_size in [20,40]:
                    for prefix in range(2):
                        learn(prefix, data_value, learningStepCount, epsilon, pop_size)
