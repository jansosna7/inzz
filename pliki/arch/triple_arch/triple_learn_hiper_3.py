...
def eval_genome(genome, config, data):
...
def eval_genome_vn(genome, config):
    data = "VehicleNumber"
    return eval_genome(genome, config, data)
...
def learn():
    ...
    for ra in range(12):
        for data_value in ["All"]:#["VehicleNumber","HaltingNumber","MeanSpeed","VehicleNumberAndHaltingNumber","HaltingNumberAndMeanSpeed","MeanSpeedAndVehicleNumber","All"]:
            ...
            config.initial_connection = random.choice(["partial_nodirect 0.33","partial_nodirect 0.5","partial_nodirect 0.66"])
            config.feed_forward = random.choice(["False","True"])
            config.compatibility_disjoint_coefficient = random.choice([0.5,0.6])
            config.bias_mutate_rate = random.choice([0.5,0.6,0.7])
            config.bias_mutate_power = random.choice([4,6])
            config.weight_mutate_rate = random.choice([0.7,0.8,0.9])
            config.weight_replace_rate = random.choice([0.5,0.6,0.7])
            config.weight_mutate_power = random.choice([1,2])
            config.compatibility_threshold = random.choice([12,15])
            config.species_elitism = random.choice([2,3])
            config.survival_threshold = random.choice([0.26,0.33,0.4])
            config.min_species_size = random.choice([9,15])
            config.max_stagnation = random.choice([10,15])

            config.activation_mutate_rate = random.choice([0,0.01,0.1,0.3])
            config.aggregation_mutate_rate = random.choice([0,0.01,0.1,0.3])
            ...
            pop.add_reporter(neat.Checkpointer(generation_interval=60, time_interval_seconds=1800, filename_prefix=os.path.join("results","neat-checkpoint"+str(seed) + "_" + str(ra)+"_"+data_value+"_"+str(learningStepCount)+"-")))
            ...
            try:
                ...
                prefix = str(seed) + "_" + str(ra) + "_"+data_value+"step_" + str(learningStepCount)+"gen_"+str(gen)
                result = eval_winner(winner,config,data_value)

                with open(os.path.join("results", f"winner{prefix}"), 'wb') as f:
                    pickle.dump(winner, f)
                    
                visualize.plot_stats(stats, ylog=False, view=False, filename=os.path.join("results", f"fitness{prefix}.svg"))
                ...
                with open("hipertuning"+str(seed)+".txt", "a") as file:
                    ...
                    file.write(str(config.num_hidden) + ",")
