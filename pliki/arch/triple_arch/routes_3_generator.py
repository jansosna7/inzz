...
for rou in range(8,16):
    nr = 1
    for begin in range(250,750,33):
        mid = 0.24
        prob = mid + random.uniform(-mid, mid)
        rows.append([nr, begin, begin+30, prob, rou])
        nr = nr + 1

for rou in range(2,8):
    nr = 1
    for begin in range(250,750,100):
        mid = 0.19
        prob = mid + random.uniform(-mid, mid)
        rows.append([nr, begin, begin+90, prob, rou])
        nr = nr + 1


sorted_array = sorted(rows, key=lambda x: x[1])

file_path = "triple_3.rou.xml"
