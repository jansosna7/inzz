...
result_f1_1 = -1*(155*harsh_result + 10) #192
result_f1_2 = -1*(155*harsh_result) #182
result_f2_1 = -1*(163*harsh_result + 5) #196
result_f2_2 = -1*(163*harsh_result) #191
result_f3_1 = -1*(176*harsh_result - 5) #201
result_f4 = -237
...
def learn():
    pop_sizes = [150]
    num_hidden = [0]
    comp_d_c = [0.3, 0.5, 0.8, 1]
    comp_w_c = [0.2, 0.5, 0.6, 0.8, 1]

    conn_add_probs = [0.5, 0.6, 0.7]
    conn_del_diff = [0.99, 0.95, 0.9, 0.8, 0.66, 0.5]
    node_add_probs = [0.05, 0.1, 0.2, 0.33, 0.4, 0.5]
    node_del_diff = [0.99, 0.95, 0.9, 0.8, 0.66, 0.5]
    activations = [0.3]
    aggregations = [0.1]
    ...
    power = [0.1, 0.25, 0.5, 0.75, 1, 1.25, 1.5, 2, 4, 6, 10]
    biases = [10,50]
    responses = [7.5]
    weights = [10, 20, 30]

    bias_mutate_rate = [0.3, 0.5]    
    bias_replace_rate = [0.1, 0.4]
    response_mutate_rate = [0.5, 0.66, 0.8, 0.9]
    weight_mutate_power = [1]
    weight_mutate_rate = [0.8]
    weight_replace_rate = [0.2, 0.3, 0.4, 0.5]
    
    compatibility = [3.5, 5, 6.5, 8]
    stagnation = [18, 25, 32]
    species_elitism = [3]
    elitism = [0, 1, 2]
    species_size = [15,16,17,18]
    survival_t = [0.5, 0.66]
    ...
    for ra in range(100):
        for i in range(1):
            ...
            config.conn_add_prob = random.choice(
                conn_add_probs) * (1 + random.uniform(-noise_small, noise_small))
            config.conn_delete_prob = random.choice(conn_del_diff) * config.conn_add_prob
            config.node_add_prob = random.choice(
                node_add_probs) * (1 + random.uniform(-noise_small, noise_small))
            config.node_delete_prob = random.choice(node_del_diff) * config.node_add_prob
            ...
            config.bias_replace_rate = random.choice(
                bias_replace_rate) * (1 + random.uniform(-noise_small, noise_small))
            ...
            config.weight_replace_rate = random.choice(
                weight_replace_rate) * (1 + random.uniform(-noise_small, noise_small))
            config.weight_mutate_rate = random.choice(
                weight_mutate_rate) * (1 + random.uniform(-noise_small, noise_small))
            config.weight_mutate_power = random.choice(
                weight_mutate_power) * (1 + random.uniform(-noise, noise))
            ...
            winner = None
            try:
                for generations in [50000/pop_size]:
                    gen += generations
