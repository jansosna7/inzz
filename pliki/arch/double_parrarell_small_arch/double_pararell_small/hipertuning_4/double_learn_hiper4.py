...
def sim(model, lenght, num, data_value):
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        for net_i in range(0, 5, 3):
            for net_j in range(0, 7, 4):
                # for every net
                xy = []
                raw_data = []
                for ix in range(1, 2):
...
def run_sim(net, distance, rou, data):
    result = -500
    try:
        result = sim(net, distance, rou, data)
...
def learn():
    ...
    num_hidden = [0, 2, 4, 8]
    comp_d_c = [0.2, 0.5, 0.8, 1, 2]
    comp_w_c = [0.01, 0.1, 0.2, 0.5, 0.6, 0.8, 1]
    ...
    power = [0.25, 0.5, 0.75, 1, 1.25, 1.5, 2, 4, 6]
    biases = [1,10,100]
    responses = [1, 10]
    weights = [1, 25]

    enabled_mutate_rate = [0, 0.01, 0.1, 0.33]

    compatibility = [3.5, 4, 4.5, 5, 5.5, 6, 7]
    stagnation = [10, 15, 30]
    species_elitism = [0, 1, 2, 3, 4, 5, 6, 8]
    elitism = [0, 1, 2, 4, 6, 8, 16]
    species_size = [8, 10, 30]
    survival_t = [0.2, 0.33, 0.5, 0.66, 0.8]
    ...
    for ra in range(100):
        for i in range(1):
            ...
            config.compatibility_disjoint_coefficient = random.choice(
                comp_d_c) * (1 + random.uniform(-noise, noise))
            config.compatibility_weight_coefficient = random.choice(
                comp_w_c) * (1 + random.uniform(-noise, noise))
            config.conn_add_prob = random.choice(
                probs) * (1 + random.uniform(-noise, noise))
            ...
            config.bias_init_stdev = m_bias / \
                random.choice(divider) * (1 + random.uniform(-noise, noise))
            ...
            winner = None
            try:
                for generations in [25000/pop_size]:
                    gen += generations
