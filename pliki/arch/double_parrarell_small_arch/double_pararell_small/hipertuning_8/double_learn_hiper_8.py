...
result_f1_1 = -1*(155*harsh_result + 10) #191
result_f1_2 = -1*(155*harsh_result) #181
result_f2_1 = -1*(163*harsh_result + 5) #195
result_f2_2 = -1*(163*harsh_result) #190
result_f3_1 = -1*(176*harsh_result - 5) #200
result_f4 = -237
...
def learn():
    ...
    for ra in range(100):
        for i in range(1):
            ...
            config.conn_delete_prob = 0.63
            config.node_add_prob = random.choice(node_add_probs) 
            config.node_delete_prob = 0.9 * config.node_add_prob
            
            config.activation_mutate_rate = 0.3
            config.aggregation_mutate_rate = 0.1
            config.enabled_mutate_rate = 0.1
            
            config.bias_replace_rate = 0.1
            config.bias_mutate_rate = 0.5 
            config.bias_mutate_power = random.choice(power)
            config.response_replace_rate = 0.1 
            config.response_mutate_rate = 0.5 
            config.response_mutate_power = 2
            config.weight_replace_rate = 0.5
            config.weight_mutate_rate = 0.8
            config.weight_mutate_power = 1
            ...
            config.bias_init_stdev = m_bias / dividers[0]
            config.response_init_stdev = m_response / dividers[1]
            config.weight_init_stdev = m_weight / dividers[2]

            config.compatibility_threshold = random.choice(compatibility)
            config.max_stagnation = random.choice(stagnation)
            config.species_elitism = 3
