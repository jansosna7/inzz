...
def run_sim(num,nettype,length):
    ...
    summary_file = "control_summaries//"+"summary_"+street_net_name+nettype+length+num+".xml"
    tripinfos_file = "control_summaries//"+"tripinfos_"+street_net_name+nettype+length+num+".xml"
    xml_content = '''<?xml version="1.0" encoding="UTF-8"?>
    <configuration xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="http://sumo.dlr.de/xsd/sumoConfiguration.xsd">
        <input>
            <net-file value="'''+street_net_name+length+nettype+'''.net.xml"/>
    ...
    filename = name + street_net_name+length+".sumocfg"
...
def learn():
    ...
    with open(filename, 'w') as file:
        for tls_type in ["_no_lights","_static","_static_offset","_actuated"]:
            for length in ["_500"]:
                results = []
                results.append(run_sim('_1', tls_type, length))
                results.append(run_sim('_2', tls_type, length))
                results.append(run_sim('_3', tls_type, length))
                results.append(run_sim('_4', tls_type, length))
