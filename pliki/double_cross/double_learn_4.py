...
def eval_genome(genome, config):
    ...
    return (fitness_4) + int(fitness_2)/100000'''


def learn(prefix, data_value):
    t1 = 259 * harsh_time
    t2 = 425 * harsh_time
    t3 = 613 * harsh_time
    t4 = 1182 * harsh_time
    ...
    PDTR = str(prefix) + "_pop_" + str(pop_size) + "_data_" + data_value
...
def repeat_learn():
    for data_value in ["All","VehicleNumberAndHaltingNumber","VehicleNumber"]:
        for prefix in [401,402]:
            try:
                learn(prefix, data_value)
            except Exception as e:
                time.sleep(5)
                print(e)
