...
nr = 0
prob = 0.074
for begin in range(0,350,50):
    for r in range(6,12):
        if((r%6) == (nr%6)):
            rows.append([nr, begin, begin+50, prob*5, r])
        else:
            rows.append([nr, begin, begin+50, prob*2, r])
    nr = nr + 1

for rou in range(12,24):
    nr = 1
    for begin in range(0,400,50):
        mid = 0.19
        prob = mid + 0.01 + random.uniform(-mid, mid)/2
        rows.append([nr, begin, begin+50, prob, rou])
        nr = nr + 1
