...
def eval_genome(genome, config):
    ...
    return (fitness_3 - 500) + int(fitness_1)/100000
...
def learn(prefix, data_value, pop_size):
    t1 = 259 * harsh_time
    t2 = 425 * harsh_time
    t3 = 613 * harsh_time
    t4 = 1182 * harsh_time
...
def repeat_learn():
    for pop_size in [100,200]:
        for data_value in ["VehicleNumber", "VehicleNumberAndHaltingNumber", "All"]:
            for prefix in [2]:
                try:
                    learn(prefix, data_value, pop_size)
                except Exception as e:
                    time.sleep(5)
                    print(e)
