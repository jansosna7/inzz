...
def learn(prefix, pop_size, data_value):
    ...
    epsilon = [0.01,0.1]
    harsh_time = [1.7,2]
    harsh_result = [1,1.05,1.25,1.5]
    initial = [0,0.5,1]
    comp_d_c = [0.2,0.4,0.5,0.6,0.8]
    comp_w_c = [0.4,0.5,0.6,0.8,1]
    conn_probs = [0.3,0.5,0.7,0.8]
    conn_diff = [0.9,0.99,1.1]
    node_probs = [0.3,0.4,0.5,0.6]
    node_diff = [0.9,0.99,1.1]
    activations = [0,0.03,0.3,0.5]
    aggregations = [0,0.03,0.3,0.5]
    bias_replace_rate = [0.1,0.2,0.3,0.5]
    bias_mutate_rate = [0.3,0.5,0.7]
    bias_mutate_power = [0.5,1,2,4]
    response_replace_rate = [0.1,0.2,0.3,0.5]
    response_mutate_rate = [0.3,0.5,0.7]
    response_mutate_power = [0.5,1,2,4]
    weight_replace_rate = [0.3,0.5,0.7]
    weight_mutate_rate = [0.5,0.7,0.9]
    weight_mutate_power = [0.5,1,2,4]
    enabled_mutate_rate = [0,0.01,0.1]
    biases = [10,50,100]
    responses = [1,5,10]
    weights = [1,10,20]
    compatibility = [10,11,12,13,14]
    stagnation = [15,20,25]
    species_elitism = [2,3,4]
    elitism = [1,2,3]
    ...
    config.conn_delete_prob = random.choice(conn_diff)
    config.node_add_prob = random.choice(node_probs)
    config.node_delete_prob = random.choice(node_diff) 
    ...
    for pop_size in [180]:
        for data_value in ["VehicleNumberAndHaltingNumber"]:
            for prefix in range(31,50):
                try:
                    learn(1000+prefix, pop_size, data_value)
                except Exception as e:
                    time.sleep(5)
                    print(e)
